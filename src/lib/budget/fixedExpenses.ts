...
/** Gastos fijos vencidos o con fecha en ≤7 días sin pagar este mes. */
export function getUrgentFixedExpenseCommitments(
  expenses: FixedExpense[],
  now: Date = new Date(),
  urgentWithinDays = 7
): FixedExpense[] {
  return sortFixedExpensesByUrgency(expenses, now).filter((expense) => {
    const status = getFixedExpenseStatus(expense, now);
